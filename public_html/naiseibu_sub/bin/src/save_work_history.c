...
int main(int argc, char **argv)
{
  ...
  puts("<html><body>");
  fp = fopen("work_history.txt", "a+");
  vars = cgiGetVariables(cgi);	/* 変数名の一覧を取得 */
  if (vars) {
      /* 変数名と復号化された値を出力 */
    fprintf(fp, "%s,%s,%s,%s,\n", cgiGetValue(cgi, vars[0]), cgiGetValue(cgi, vars[1]), cgiGetValue(cgi, vars[2]), cgiGetValue(cgi, vars[3]));
  }
 printf("登録完了<br>");
 printf("<input type=\"button\" value=\"閉じる\" onclick=\"self.close();\" onkeypress=\"self.close();\">");
